...
/**
 * Describe:RxHelperBase
 * Created by liying on 2018/3/5
 */
public class RxHelperBase {
     ...
     * @return
     */
    public static <T> Observable.Transformer<HttpResultHomeDemo<T>, T> handleResult(final ActivityLifeCycleEvent event, final PublishSubject<ActivityLifeCycleEvent> lifecycleSubject) {
        return new Observable.Transformer<HttpResultHomeDemo<T>, T>() {
            @Override
            public Observable<T> call(Observable<HttpResultHomeDemo<T>> tObservable) {
                Observable<ActivityLifeCycleEvent> compareLifecycleObservable =
                        lifecycleSubject.takeFirst(new Func1<ActivityLifeCycleEvent, Boolean>() {
                            @Override
                            public Boolean call(ActivityLifeCycleEvent activityLifeCycleEvent) {
                ...
                return tObservable.flatMap(new Func1<HttpResultHomeDemo<T>, Observable<T>>() {
                    @Override
                    public Observable<T> call(HttpResultHomeDemo<T> result) {
                        if (!result.isError()) {
