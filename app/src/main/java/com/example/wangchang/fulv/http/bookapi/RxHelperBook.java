...
public class RxHelperBook {
     ...
     * @return
     */
    public static <T> Observable.Transformer<HttpResultBookDemo<T>, T> handleResult(final ActivityLifeCycleEvent event, final PublishSubject<ActivityLifeCycleEvent> lifecycleSubject) {
        return new Observable.Transformer<HttpResultBookDemo<T>, T>() {
            @Override
            public Observable<T> call(Observable<HttpResultBookDemo<T>> tObservable) {
                Observable<ActivityLifeCycleEvent> compareLifecycleObservable =
                            lifecycleSubject.takeFirst(new Func1<ActivityLifeCycleEvent, Boolean>() {
                            @Override
                            public Boolean call(ActivityLifeCycleEvent activityLifeCycleEvent) {
                ...
                return tObservable.flatMap(new Func1<HttpResultBookDemo<T>, Observable<T>>() {
                    @Override
                    public Observable<T> call(HttpResultBookDemo<T> result) {
