...
public class HttpLoggingInterceptor implements Interceptor {
    ...
    @Override public Response intercept(Interceptor.Chain chain) throws IOException {
        ...
        String requestStartMessage = "--> 开始请求 " + request.method() + ' ' + request.url() + ' ' + protocol;
        ...
        if (logHeaders) {
            ...
            for (int i = 0, count = headers.size(); i < count; i++) {
                ...
                if (!"Content-Type".equalsIgnoreCase(name) && !"Content-Length".equalsIgnoreCase(name)) {
                    logger.log("请求Header -->" + name + ": " + headers.value(i));
                }
            }

            if (!logBody || !hasRequestBody) {
                logger.log("--> 结束请求" + request.method());
            } else if (bodyEncoded(request.headers())) {
                logger.log("--> 结束请求" + request.method() + " (encoded body omitted)");
            } else {
                ...
                if (isPlaintext(buffer)) {
                    ...
                    logger.log("--> 结束请求 "  + request.method() + " (binary "
        ...
        logger.log("<-- " + "响应码：" + response.code() + ' ' + "响应Message：" + response.message() + ' '
                + "\n响应地址："  + response.request().url() + "  响应时间 (" + tookMs + "ms" + (!logHeaders ? ", "
                + bodySize + " body" : "") + ')');

        if (logHeaders) {
            Headers headers = response.headers();
            for (int i = 0, count = headers.size(); i < count; i++) {
                logger.log("响应Header-->" +  headers.name(i) + ": " + headers.value(i));
            }

            if (!logBody || !HttpHeaders.hasBody(response)) {
                logger.log("<-- 结束 HTTP");
            } else if (bodyEncoded(response.headers())) {
                logger.log("<-- 结束 HTTP (encoded body omitted)");
            } else {
                ...
                if (!isPlaintext(buffer)) {
                    logger.log("");
                    logger.log("<-- 结束 HTTP (binary " + buffer.size() + "-byte body omitted)");
                    return response;
                }
                ...
                logger.log("<-- 结束 HTTP (" + buffer.size() + "-byte body)");
            }
        }
