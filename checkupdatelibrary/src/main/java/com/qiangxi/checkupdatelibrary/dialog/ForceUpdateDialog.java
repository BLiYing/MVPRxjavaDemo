...
public class ForceUpdateDialog extends Dialog {
    ...
    private void initData() {
        ...
        if (TextUtils.isEmpty(mAppTime)) {
            ...
            forceUpdateTime.setText(context.getString(R.string.updatelibrary_pushtime)+ ":" + mAppTime);
        }
        //新版版本名,eg:2.2.1
        if (TextUtils.isEmpty(mVersionName)) {
            forceUpdateVersion.setVisibility(View.GONE);
        } else {
            forceUpdateVersion.setText(context.getString(R.string.updatelibrary_versionname)+ ":" + mVersionName);
        }
        //新版本app大小
        if (mAppSize == 0) {
            forceUpdateSize.setVisibility(View.GONE);
        } else {
            forceUpdateSize.setText(context.getString(R.string.updatelibrary_size)+ ":" + mAppSize + "M");
        }
        //更新日志
    ...
    private void setNetWorkState() {
        if (NetWorkUtil.isWifiConnection(context)) {
            forceUpdateNetworkState.setText(context.getString(R.string.updatelibrary_onlywifi));
            forceUpdateNetworkState.setTextColor(Color.parseColor("#629755"));
        } else if (NetWorkUtil.isMobileConnection(context)) {
            forceUpdateNetworkState.setText(context.getString(R.string.updatelibrary_mobile));
            forceUpdateNetworkState.setTextColor(Color.parseColor("#BAA029"));
        } else if (!NetWorkUtil.hasNetConnection(context)) {
            forceUpdateNetworkState.setText(context.getString(R.string.updatelibrary_retry));
    ...
    public void download() {
        ...
        if (context.getString(R.string.updatelibrary_install).equals(forceUpdate.getText().toString().trim())) {
        ...
        HttpRequest.download(mDownloadUrl, mFilePath, mFileName, new DownloadCallback() {
            ...
            public void onDownloadSuccess(File file) {
                ...
                forceUpdate.setText(context.getString(R.string.updatelibrary_install));
                ApplicationUtil.installApk(context, file);
            }
            ...
            public void onProgress(long currentProgress, long totalProgress) {
                ...
                forceUpdate.setText(context.getString(R.string.updatelibrary_isdownloading));
            ...
            public void onDownloadFailure(String failureMessage) {
                forceUpdate.setEnabled(true);
                forceUpdate.setText(context.getString(R.string.updatelibrary_retrydownloading));
            }
        });
    }
